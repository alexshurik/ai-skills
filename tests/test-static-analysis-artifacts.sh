...
for tool in jscpd lizard semgrep complexipy radon vulture bandit; do
  printf '%s\n' \
    '#!/bin/sh' \
...
grep -q 'SUMMARY: 8 OK' "$FIXTURE_DIR/summary.txt"
...
test "$(find "$FIXTURE_DIR/logs" -type f -name '*.log' | wc -l | tr -d ' ')" -eq 8
